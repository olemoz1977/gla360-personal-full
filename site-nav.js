(function(){
  ...
  function resolveLang(){
    ...
    if(window.Leadership360I18n && typeof window.Leadership360I18n.current==='function'){
      return normalise(window.Leadership360I18n.current());
    }
    return normalise(storedLang()||document.documentElement.lang||navigator.language||'lt');
  }

  const COPY={
    lt:{aria:'Pagrindinė navigacija',switchLabel:'EN',switchAria:'Switch to English',items:[
  ...
  if(!document.getElementById('leadership360-site-nav-style')){
    const style=document.createElement('style');
    style.id='leadership360-site-nav-style';
    style.textContent=`
  ...
  function render(forcedLang){
    ...
    nav.innerHTML='<div class="site-nav__inner">'+copy.items.map(([href,label])=>{
      ...
      return `<a href="${hrefWithLang(href,lang)}"${active?' class="is-active" aria-current="page"':''}>${label}</a>`;
    ...
    nav.querySelector('.site-nav__lang').addEventListener('click',()=>{
      ...
      window.dispatchEvent(new CustomEvent('leadership360:languagechange',{detail:{lang:next}}));
      const u=new URL(location.href);
      u.searchParams.set('lang',next);
      if(u.hash){
        const raw=String(u.hash).replace(/^#/,'');
        const qs=new URLSearchParams(raw);
        if(qs.has('lang')){qs.set('lang',next);u.hash=qs.toString()}
      }
